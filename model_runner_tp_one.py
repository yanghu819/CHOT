...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        if model_input.is_prompt:
            ...
            embedding_dim = self.model.lm_head.embedding_dim
            device = hidden_or_intermediate_states.device
            dtype = hidden_or_intermediate_states.dtype

            self.ptuning_params = torch.zeros(embedding_dim, device=device, dtype=dtype)
            ...
            # 优雅地获取本地的 lm_head 权重分片 (循环外获取一次即可)
            # 并转换为bfloat16以保证计算效率和稳定性
            local_lm_head_weight_shard = self.model.lm_head.weight.to(dtype=torch.bfloat16)
            
            # ----------------------------------------------------------------------
            # 2. 开始优化循环
            # ----------------------------------------------------------------------
            for _ in range(chot_steps):
                # -- Step 2.1: 前向传播 (Forward Pass) --
                hidden_states_cur = hidden_or_intermediate_states + self.ptuning_params
                
                # 计算部分 logits (在本地分片上)
                partial_logits = torch.matmul(hidden_states_cur.to(torch.bfloat16), local_lm_head_weight_shard.T)

                # 使用 all_gather 聚合成完整的 logits
                if tp_size > 1:
                    from vllm.distributed import tensor_model_parallel_all_gather
                    full_logits = tensor_model_parallel_all_gather(partial_logits)
                else:
                ...
                del max_logits, exp_logits, sum_exp_logits, shift_logits
                
                # -- Step 2.3: 手动反向传播 (Manual Backward Pass) --
                # 准备 target labels
                target_tokens = model_input.input_tokens
                shift_labels = target_tokens[1:].contiguous()

                # 计算 dL/dLogits (即 probs - one_hot)
                vocab_size = probs.shape[-1]
                one_hot_labels = torch.zeros_like(probs)
                one_hot_labels.scatter_(1, shift_labels.unsqueeze(1), 1)

                d_logits_grad = probs - one_hot_labels
                del probs, one_hot_labels

                # 根据有效token数量进行归一化
                num_valid_tokens = shift_labels.numel()
                d_logits_grad /= num_valid_tokens
                del shift_labels, target_tokens
                
                # 计算 dL/dHidden_states (dL/dLogits * W)
                d_hidden = None
                if tp_size > 1:
                    from vllm.distributed import split_tensor_along_last_dim, tensor_model_parallel_all_reduce
                    # 将完整的梯度 d_logits_grad 切分，以匹配本地的权重分片
                    local_d_logits_grad, _ = split_tensor_along_last_dim(d_logits_grad, tp_size)
                    # 计算部分梯度
                    partial_d_hidden = torch.matmul(local_d_logits_grad, local_lm_head_weight_shard)
                    # 使用 all_reduce 聚合所有部分的梯度
                    d_hidden = tensor_model_parallel_all_reduce(partial_d_hidden)
                    del local_d_logits_grad, partial_d_hidden
                else:
                    d_hidden = torch.matmul(d_logits_grad, local_lm_head_weight_shard)
                del d_logits_grad

                # -- Step 2.4: 聚合梯度并用Adam更新 --
                d_ptuning = d_hidden.mean(dim=0)
                del d_hidden

                adam_step_counter += 1
                bias_correction1 = 1 - beta1 ** adam_step_counter
                bias_correction2 = 1 - beta2 ** adam_step_counter
                
                # 更新一阶和二阶动量估值
                self.adam_m.mul_(beta1).add_(d_ptuning, alpha=1 - beta1)
                self.adam_v.mul_(beta2).addcmul_(d_ptuning, d_ptuning, value=1 - beta2)
                ...
                self.ptuning_params.add_(update, alpha=-chot_lr)
                if weight_decay > 0:
                    self.ptuning_params.add_(self.ptuning_params, alpha=-chot_lr * weight_decay)
                
                del d_ptuning, m_hat, v_hat, update
