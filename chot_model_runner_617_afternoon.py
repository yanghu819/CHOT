...
class ModelRunner(GPUModelRunnerBase[ModelInputForGPUWithSamplingMetadata]):
    ...
    def execute_model(
        ...
        # [逻辑 1/4] 清理已完成请求的Delta (已实现)
        if model_input.finished_requests_ids:
            for req_id in model_input.finished_requests_ids:
                if req_id in model_input.request_ids_to_seq_ids:
                    seq_ids_to_remove = model_input.request_ids_to_seq_ids[req_id]
                    for seq_id in seq_ids_to_remove:
                        self.request_deltas.pop(seq_id, None)
        ...
        } if hasattr(self, 'has_inner_state') and self.has_inner_state else {}
        ...
        # =====================================================================================
        # Prefill阶段：计算、保存并应用每个请求的Delta
        # =====================================================================================
        if model_input.is_prompt and prefill_meta is not None and prefill_meta.seq_lens:
            if hidden_or_intermediate_states.shape[0] < 20000:
                num_prefill_seqs = len(prefill_meta.seq_lens)
                prefill_seq_groups_to_sample = model_input.sampling_metadata.seq_groups[:num_prefill_seqs]

                if prefill_seq_groups_to_sample:
                    seq_id_to_req_id_map = {
                        seq_id: req_id
                        for req_id, seq_ids in model_input.request_ids_to_seq_ids.items()
                        for seq_id in seq_ids
                    }

                    print("\n==================== [SLOT REAL-TIME TRACE] ====================")
                    print(f"Executing SLOT optimization for a new batch of {len(prefill_seq_groups_to_sample)} prompts.")
                    
                    for i, sg_to_sample in enumerate(prefill_seq_groups_to_sample):
                        if sg_to_sample.seq_ids:
                            primary_seq_id = sg_to_sample.seq_ids[0]
                            req_id = seq_id_to_req_id_map.get(primary_seq_id, "UNKNOWN_REQUEST_ID")
                            prompt_len = prefill_meta.seq_lens[i]
                            print(f"  -> Optimizing for Request ID: {req_id}, Prompt Length: {prompt_len}")
                    
                    print("-------------------- [SLOT Loss Trace] --------------------")

                prefill_seq_lens_tensor = torch.tensor(prefill_meta.seq_lens, device=self.device, dtype=torch.long)
                token_to_seq_idx = torch.repeat_interleave(
                    torch.arange(num_prefill_seqs, device=self.device),
                    repeats=prefill_seq_lens_tensor
                )
                
                embedding_dim = self.model.config.hidden_size
                ptuning_params = torch.zeros((num_prefill_seqs, embedding_dim), device=self.device, dtype=hidden_or_intermediate_states.dtype)
                adam_m = torch.zeros_like(ptuning_params)
                adam_v = torch.zeros_like(ptuning_params)
                ...
                if chot_steps > 0:
                    shifted_token_to_seq_idx = token_to_seq_idx[:-1]
                    # token_counts 在循环外计算一次即可
                    token_counts = torch.bincount(shifted_token_to_seq_idx, minlength=num_prefill_seqs).to(hidden_or_intermediate_states.dtype)

                    for adam_step in range(1, chot_steps + 1):
                        hidden_states_cur = hidden_or_intermediate_states + ptuning_params.index_select(0, token_to_seq_idx)
                        ...
                        valid_labels = torch.clamp(shift_labels, min=0)
                        valid_label_mask = (shift_labels >= 0).to(probs.dtype)
                        one_hot.scatter_(1, valid_labels.unsqueeze(1), valid_label_mask.unsqueeze(1))
                        
                        # [!!] 核心修改：计算per-token loss并分别聚合
                        masked_probs = probs[torch.arange(probs.size(0)), valid_labels] * valid_label_mask
                        # 计算每个token的loss，而不是总loss
                        per_token_losses = -torch.log(masked_probs + 1e-9)
                        
                        # 1. 仍然计算总loss用于梯度更新
                        loss = torch.sum(per_token_losses) / (torch.sum(valid_label_mask) + 1e-9)
                        
                        # 2. 计算并打印每个prompt的独立loss
                        if prefill_seq_groups_to_sample:
                            per_seq_loss_sum = torch.zeros(num_prefill_seqs, device=self.device, dtype=per_token_losses.dtype)
                            per_seq_loss_sum.scatter_add_(0, shifted_token_to_seq_idx, per_token_losses)
                            # 加上一个极小值避免除以零
                            per_seq_avg_loss = per_seq_loss_sum / (token_counts + 1e-9)
                            
                            print(f"  [Adam Step: {adam_step}/{chot_steps}]")
                            for i, sg_to_sample in enumerate(prefill_seq_groups_to_sample):
                                if sg_to_sample.seq_ids:
                                    primary_seq_id = sg_to_sample.seq_ids[0]
                                    req_id = seq_id_to_req_id_map.get(primary_seq_id, "UNKNOWN")
                                    loss_value = per_seq_avg_loss[i].item()
                                    print(f"    -> Req ID: {req_id} | Loss: {loss_value:.4f}")

                        # --- 后续梯度计算和优化步骤保持不变 ---
                        # d_logits_grad = (probs - one_hot) / (torch.sum(valid_label_mask) + 1e-9)
                        d_logits_grad = probs - one_hot

                        d_hidden = torch.matmul(d_logits_grad, lm_head_weight)
                        
                        d_ptuning_batch = torch.zeros_like(ptuning_params)
                        d_ptuning_batch.index_add_(0, shifted_token_to_seq_idx, d_hidden)
                        d_ptuning_batch = d_ptuning_batch / (token_counts.unsqueeze(1) + 1e-9)

                        bias_correction1 = 1 - beta1**adam_step
                        bias_correction2 = 1 - beta2**adam_step
                        adam_m = beta1 * adam_m + (1 - beta1) * d_ptuning_batch
                        adam_v = beta2 * adam_v + (1 - beta2) * (d_ptuning_batch**2)
                        m_hat = adam_m / bias_correction1
                        v_hat = adam_v / bias_correction2
                        update = chot_lr * m_hat / (torch.sqrt(v_hat) + eps)
                        ptuning_params = ptuning_params - update - chot_lr * weight_decay * ptuning_params
                    ...
                    for i, sg_to_sample in enumerate(prefill_seq_groups_to_sample):
                        if sg_to_sample.seq_ids:
                            primary_seq_id = sg_to_sample.seq_ids[0]
                            self.request_deltas[primary_seq_id] = ptuning_params[i]
                
                final_deltas_for_tokens = ptuning_params.index_select(0, token_to_seq_idx)
                hidden_or_intermediate_states = hidden_or_intermediate_states + final_deltas_for_tokens
        
        # ... (Decode阶段和返回逻辑与之前版本完全相同，此处省略) ...
        elif not model_input.is_prompt and decode_meta is not None and decode_meta.seq_lens:
            num_decode_seqs = len(decode_meta.seq_lens)
            num_prefill_seqs_in_batch = len(prefill_meta.seq_lens) if prefill_meta is not None else 0
            
            decode_seq_groups_to_sample = model_input.sampling_metadata.seq_groups[num_prefill_seqs_in_batch : num_prefill_seqs_in_batch + num_decode_seqs]
            
            current_decode_batch_size = hidden_or_intermediate_states.shape[0]
            embedding_dim = hidden_or_intermediate_states.shape[1]
            deltas_to_apply = torch.zeros((current_decode_batch_size, embedding_dim), device=self.device, dtype=hidden_or_intermediate_states.dtype)
            
            applied_seq_ids = []
            for i, sg_to_sample in enumerate(decode_seq_groups_to_sample):
                if sg_to_sample.seq_ids:
                    primary_seq_id = sg_to_sample.seq_ids[0]
                    if primary_seq_id in self.request_deltas:
                        if i < deltas_to_apply.shape[0]:
                             deltas_to_apply[i] = self.request_deltas[primary_seq_id]
                             applied_seq_ids.append(primary_seq_id)
            
            hidden_or_intermediate_states = hidden_or_intermediate_states + deltas_to_apply
        ...
        if self.return_hidden_states:
            ...
            elif decode_meta is not None and decode_meta.use_cuda_graph:
