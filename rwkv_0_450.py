## Correct: 6323 - Total: 14042 - Accuracy: 0.45029: 100%|███████████████████████| 14042/14042 [07:08<00:00, 32.78it/s]
...
if os.environ.get('RWKV_V7_ON') == '1':
    ...
    class RWKV_x070(MyModule):
        def __init__(self, model, strategy):
            ...
            self.glot_feature = torch.zeros(1, self.n_embd, device=DEVICE, dtype=DTYPE, requires_grad=True)
        ...
        def forward_seq(self, idx:List[int], state:List[torch.Tensor], full_output:bool=False):
            with torch.no_grad(): 
                ...
                if os.environ.get("IS_PROMPT") == "True":
                    with torch.enable_grad():
                        glot_feature = torch.nn.Parameter(
                            0.00001*torch.randn(1, x.shape[-1], device=x.device, dtype=x.dtype, requires_grad=True)
                        )
                        
                        optim = torch.optim.AdamW([glot_feature], lr=0.0001, eps=1e-5, weight_decay=1e-3)
                        
                        # 设置优化步数和窗口数量
                        num_steps = 3
                        num_chunks = num_steps
                        seq_len = len(idx) - 1  # 减1是因为我们需要预测下一个token
                        
                        for i in range(num_steps):
                            optim.zero_grad()
                            
                            # 计算当前步骤应该关注的窗口位置
                            # 将序列均匀分成num_chunks个部分，取每个部分的中心点
                            ...
                            # 为当前窗口创建掩码
                            mask = torch.zeros(seq_len, device=x.device, dtype=torch.bool)
                            mask[start_idx:end_idx] = True
                            
                            x_feature = x + glot_feature/glot_feature.norm()
                            
                            logits = x_feature @ z['head.weight']
                            shift_logits = logits.contiguous()[:-1]
                            shift_labels = torch.tensor(idx[1:], device=x.device, dtype=torch.long)
                            
                            # 只计算当前窗口的损失
                            loss = F.cross_entropy(
                                shift_logits[mask].view(-1, shift_logits.size(-1)), 
                                shift_labels[mask].view(-1)
                            )
                            loss.backward()
                            
                            # 打印详细的优化信息
                            print(f"\n优化步骤 {i+1}/{num_steps}:")
                            print(f"窗口范围: {start_idx} 到 {end_idx}")
                            print(f"损失: {loss.item():.4f}")
                            if glot_feature.grad is not None:
                                grad_norm = glot_feature.grad.norm().item()
                                print(f"梯度范数: {grad_norm:.4f}")
                                print(f"最大梯度: {glot_feature.grad.max().item():.4f}")
                                print(f"最小梯度: {glot_feature.grad.min().item():.4f}")
                            else:
                                print("警告: 没有梯度!")
                            
                            print(f"步骤前特征范数: {glot_feature.norm().item():.4f}")
                            
                            optim.step()
                            
                            print(f"步骤后特征范数: {glot_feature.norm().item():.4f}")
                            print(f"特征最大值: {glot_feature.max().item():.4f}")
                            print(f"特征最小值: {glot_feature.min().item():.4f}")
                            
                        self.glot_feature = glot_feature/glot_feature.norm()
                        x = x + self.glot_feature
                        os.environ["IS_PROMPT"] = "False"
                else:
                    # import ipdb; ipdb.set_trace()
                    x = x + self.glot_feature
