        ...
        if model_input.is_prompt:
            ...
            if 1: 
                ...
                try:
                    ...
                    self.ptuning_params = 0.00001*torch.randn(
                        hidden_or_intermediate_states.shape[1],
                    ...
                    for step in range(chot_steps):
                        ...
                        # --- EDIT START ---
                        # 定义要忽略的 Vision Token ID (根据您的模型调整)
                        # 这里使用您示例中频繁出现的 151656 作为例子
                        # 更稳健的方法可能是检查一个范围或使用分词器信息
                        VISION_TOKEN_ID = 151656

                        # 创建掩码，仅包含有效标签（非负）且非 Vision Token 的位置
                        # 原始掩码: valid_label_mask = (flat_labels >= 0).float()
                        combined_mask = ((flat_labels >= 0) & (flat_labels != VISION_TOKEN_ID)).float()
                        # --- EDIT END ---
                        ...
                        try:
                            ...
                            # 直接计算 masked_probs，并应用新的 combined_mask
                            masked_probs = flat_probs[batch_indices, valid_labels] * combined_mask # 使用 combined_mask
                            ...
                            num_valid = torch.sum(combined_mask) # 使用 combined_mask 计算有效数量
                            loss = -torch.sum(log_probs) / (num_valid + 1e-10)
                            # print(f"DEBUG - loss: {loss.item()}") # Removed
                            # 及时删除中间变量
                            del batch_indices, valid_labels, masked_probs, log_probs, num_valid # 保留 combined_mask 用于梯度计算
                        except Exception as e:
                        ...
                        # 将 combined_mask 转换为与 one_hot 相同的数据类型
                        combined_mask_cast = combined_mask.to(dtype=one_hot.dtype) # 使用 combined_mask
                        ...
                        # print(f"combined_mask_cast.unsqueeze(1).shape: {combined_mask_cast.unsqueeze(1).shape}, "
                        #     f"dtype: {combined_mask_cast.unsqueeze(1).dtype}")
                        # === DEBUG INFO END === (Removed Block)
                        
                        try:
                            # print("DEBUG - Attempting scatter operation") # Removed
                            one_hot.scatter_(1, valid_labels.unsqueeze(1), combined_mask_cast.unsqueeze(1)) # 使用 combined_mask_cast
                        ...
                        try:
                            ...
                            # 调整梯度的缩放, 使用 combined_mask 的和
                            num_valid_grad = torch.sum(combined_mask) # 使用 combined_mask 计算有效数量
                            d_logits_grad = d_logits_grad / (num_valid_grad + 1e-10)
                            # print(f"DEBUG - d_logits_grad after scaling: {d_logits_grad.shape}") # Removed
                            # 删除不再需要的 one_hot 和 probs
                            del one_hot, probs, combined_mask, combined_mask_cast, num_valid_grad # 清理掩码变量
                        except Exception as e:
                        ...
                        try:
                            ...
                            lm_head_weight = self.model.language_model.lm_head.weight  # [vocab_size, hidden_dim]
